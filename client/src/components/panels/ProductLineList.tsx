...
const ARCHETYPES: Archetype[] = ['student', 'planner', 'daily'];
...
const archetypeThumb = (archetype: Archetype): string =>
  ((A.notebook as any)[archetype]?.hardcover_ring) ?? '';
    ...
    <section className="flex flex-col gap-2">
      <header className="flex items-center justify-between gap-2">
        ...
        <span className="text-[16px] text-text-2">
          <span className="font-bold text-text">{lines.length}</span>{' '}
      ...
      <div>
        {lines.length === 0 ? (
          <div className="py-3 text-[17px] text-text-2">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 pl-1 pr-2 py-2 flex flex-col gap-1.5">
                    {renameId === line.id ? (
                      <input
                        ...
                        className="w-full text-[16px] font-bold bg-surface-2 border border-border-soft px-1.5 py-0.5"
                        maxLength={32}
                      />
                    ) : (
                      // TITLE — biggest, boldest, darkest ink.
                      <div
                        className="text-[16px] font-bold text-text truncate leading-tight pr-12"
                    ...
                    <div className="text-[12px] font-medium text-text-3 truncate leading-tight -mt-0.5 pr-12">
                      {archetypeLabel(line.archetype)} Notebook
                    </div>
                    {/* TAG — the market genre, the quietest level: tiny caps
                        pill, subordinate to the title (not a second heading). */}
                    {line.genre && (
                      <div className="mt-auto">
                        <span className="inline-flex items-center px-1.5 py-px border border-border-soft bg-surface-2 text-[9.5px] uppercase tracking-wider font-semibold text-text-3">
                          {line.genre}
                        </span>
                      </div>
                    )}
                  ...
                  {isActive && (
                    <span
                      aria-hidden
                      className="absolute top-0.5 left-0.5 bg-primary text-[13px] font-bold tracking-wider px-1 py-px border border-border z-10"
                      style={{ color: '#FAF7E8' }}
                    >
                      ACTIVE
                    </span>
      ...
      <div className="sticky bottom-0 z-10 -mx-3.5 -mb-3.5 mt-1 px-3.5 pt-2 pb-3 bg-surface border-t-2 border-border-soft">
        <button
          ...
          className={clsx(
            'w-full h-[36px] inline-flex items-center justify-center gap-1.5 border-2 border-dashed text-[17px] font-bold cursor-pointer transition-colors',
            canAdd
        ...
        <div className="mt-1.5 px-2 py-0.5 text-[15px] text-text-3 leading-snug">
      ...
      {addOpen && addRect &&
        createPortal(
          ...
          >
            <div className="px-3 py-2 border-b border-border-soft text-[15px] uppercase tracking-wider text-text-3 bg-surface-2">
            ...
            {ARCHETYPES.map((arch) => (
              ...
              >
                ...
                <div className="flex flex-col min-w-0">
                  <span className="text-[17px] font-bold text-text leading-tight">
                    {archetypeLabel(arch)} Notebook
                  </span>
                  <span className="text-[15px] text-text-3 leading-snug mt-0.5">
                    {ARCH_DESC[arch]}
                  </span>
                </div>
              </button>
          ...
          >
            ...
            >
              <header className="flex items-center gap-2 px-4 py-3 border-b-2 border-border-soft bg-surface-2">
                ...
                <span className="text-[18px] uppercase tracking-wider font-bold text-text">
                  Delete this notebook?
                </span>
              </header>
              <div className="px-4 py-4 text-[17px] text-text-2 leading-relaxed">
                <p>
                  This will remove <span className="font-bold text-text">{confirmLine.name}</span>
              ...
              <footer className="flex items-center justify-end gap-2 px-4 py-3 border-t-2 border-border-soft bg-surface-2/70">
                <button
                  type="button"
                  onClick={cancelDelete}
                  className="h-[34px] px-4 border-2 border-border-soft bg-surface text-text text-[17px] uppercase tracking-wider font-bold hover:bg-surface-2 cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="h-[34px] px-4 border-2 border-border bg-danger text-white text-[17px] uppercase tracking-wider font-bold hover:-translate-y-px hover:shadow-[2px_2px_0_0_var(--c-shadow)] cursor-pointer inline-flex items-center gap-1.5"
                  style={{ color: '#FAF7E8' }}
                >
