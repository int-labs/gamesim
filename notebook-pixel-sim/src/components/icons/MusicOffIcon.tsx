...
/**
 * Custom "music off" glyph — a music note with a slash through it.
 * Imported from a designer-supplied SVG (~/Downloads/music-off-icon.svg).
 *
 * Inlined as a React component (rather than an `<img>` reference) so it
 * picks up `currentColor` and matches the tone of the surrounding HUD
 * icon button — same color/size API as a lucide icon.
 */
export function MusicOffIcon({ size = 16, color = 'currentColor', className }: Props) {
  return (
    <svg
