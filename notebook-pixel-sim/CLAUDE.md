...
- **Two colour-class traps, both of which fail silently.**
  - **The text weight is `text-success`, not `text-success-ink`.** `textColor` maps `success`/`warning`/`danger`/`info` *straight* to the darker `--c-*-ink` weights, so the plain name is already the readable one. `text-success-ink` is not a class, compiles to nothing, and leaves the value on the default ink — which looks like a design that forgot its colour coding rather than a typo.
  - **`/NN` alpha modifiers only work because of `alphaVar()`.** Every token is a CSS variable, and Tailwind cannot inject an alpha into an opaque `var()` — it emits an invalid colour and the browser falls back to *transparent*. That is how ~50 call sites (`bg-warning-soft/40`, `border-success/50`, …) came to render no tint at all, for their whole life, with no build warning. `alphaVar()` in the config returns `color-mix()` when an alpha is requested. Keep every new colour token wrapped in it, and keep its guards: for an un-suffixed utility Tailwind passes `var(--tw-bg-opacity)` rather than a number, so anything non-finite must fall through to the plain variable or the un-suffixed classes break instead.
- **Border weight is an affordance, not decoration** (RULE 5 in `styles/index.css`): a 2px frame means the thing is pressable, and containers/cards/readouts get a hairline or nothing. Exemptions are documented beside the rule — segmented controls and tab strips (Product/Business, Focus/Shelf) share one frame and carry no per-segment shadow. Both directions are worth auditing in the browser after a styling change, since a readout that keeps its frame reads as a dead button.
