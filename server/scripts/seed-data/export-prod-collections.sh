...
# Move into the "base" directory where JSON files are written
cd "${SCRIPT_DIR}/base"

# Expect PROD_MONGO_URI_WITH_DB to be set in the environment
if [[ -z "${PROD_MONGO_URI_WITH_DB:-}" ]]; then
  echo "ERROR: PROD_MONGO_URI_WITH_DB environment variable is not set."
  echo "Example:"
  echo '  export PROD_MONGO_URI_WITH_DB="mongodb+srv://user:pass@cluster/dbname"'
  exit 1
fi

URI="${PROD_MONGO_URI_WITH_DB}"
